...
- [x] **3.1 `scripts/setup.sh` end to end** on a fresh repo. *Accept:* Step 1 completes with no manual edits except CLAUDE.md placeholders. **Done 2026-09-06** against a genuinely fresh repo (`bridge-setup-test`). Ran clean, needed no manual edits, and is idempotent on a second run. Found and fixed three things — see the commit. **Not covered:** the run used a dummy `CLAUDE_CODE_OAUTH_TOKEN`, so no live Claude run happened on that repo; the live path is evidenced on `claude-bridge-trial` instead, and a real token on a fresh repo is task 3.4's job.
